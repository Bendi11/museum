...
pub fn start() {
    ...
    app
        ...
        .add_startup_system(load.before(setup))
        //.add_startup_system(setup)
        .add_system(input.before(kb_events))
        .add_system(kb_events)
        .run();
}
...
fn load(
    ...
    textures.floor = assets.load("floor.png");  
}
...
/// Resource containing handles to all textures
struct Textures {
    floor: Handle<Image>,
}
